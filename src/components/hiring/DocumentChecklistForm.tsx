  ...
  useEffect(() => {
    ...
    if (saved) {
      setForm(current => ({
        ...
        overallStatus: saved.overallStatus || current.overallStatus,
      }));
    ...
    <div className="mx-auto max-w-[1500px] pb-10">
      <div className="border-b-2 border-[#0d3c68] px-1 pb-2 flex items-center justify-between no-print">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => window.open(`/dashboard/hiring/${candidateId}/print/doc-checklist`, '_blank')}>Print Form</Button>
                ...
                <div className="space-y-2">
                  ...
                  <Field title="Overall Checklist Status (Manual Override)">
                    <select className={inputClass} value={form.overallStatus} onChange={(e) => set({ overallStatus: e.target.value })}>
                      <option value="Incomplete">Incomplete (Keep in this step)</option>
                      <option value="Complete">Complete (Advance to next step)</option>
                      <option value="Verified">Verified (Advance to next step)</option>
                    </select>
                  </Field>
                </div>
              </div>
