...
export default function InterviewWorkspace({ view }: { view: View }) {
  ...
  const [form, setForm] = useState({ candidateId: '', interviewerId: '', roundType: meta.rounds?.[0] || 'Telephonic', scheduledDate: '', mode: 'Video', location: '', meetingLink: '' });
  ...
  const params = {
    page,
    limit: PAGE_SIZE,
    ...(status !== 'All' && view !== 'new' && view !== 'statistics' ? { status } : {}),
    ...(round !== 'All' && view !== 'new' && view !== 'statistics' ? { roundType: round } : {}),
    ...(query.trim() && view !== 'new' && view !== 'statistics' ? { search: query.trim() } : {}),
  };
  ...
  const schedule = useMutation({
    ...
    onSuccess: () => {
      refresh();
      setForm({ candidateId: '', interviewerId: '', roundType: meta.rounds?.[0] || 'Telephonic', scheduledDate: '', mode: 'Video', location: '', meetingLink: '' });
    },
  });
  const saveFeedback = useMutation({
    ...
    onSuccess: () => {
      refresh();
      setFeedbackId(null);
      setFeedback({ status: 'Completed', rating: 3, feedback: '' });
    },
  });

  const visible = useMemo(() => interviews.filter((item: any) => !meta.rounds || meta.rounds.includes(item.roundType)), [interviews, meta.rounds]);
  const metrics = useMemo(() => {
    const completed = visible.filter((i: any) => i.status === 'Completed').length;
    const scheduled = visible.filter((i: any) => i.status === 'Scheduled').length;
    const rated = visible.filter((i: any) => typeof i.rating === 'number');
    const average = rated.length ? (rated.reduce((sum: number, i: any) => sum + i.rating, 0) / rated.length).toFixed(1) : '-';
    return [{ label: 'Total interviews', value: pageMeta.total || visible.length }, { label: 'Scheduled', value: scheduled }, { label: 'Completed', value: completed }, { label: 'Average rating', value: average }];
  }, [pageMeta.total, visible]);
  ...
  return <div className="mx-auto flex max-w-[1500px] flex-col gap-4 pb-8">
    <div className="border-b border-zinc-100 pb-3 dark:border-zinc-800">
      <p className="text-[11px] font-medium uppercase tracking-wider text-indigo-600">Hiring process · interviews</p>
      <h1 className="mt-1 text-xl font-medium text-zinc-900 dark:text-zinc-50">{meta.title}</h1>
    ...
    {view === 'new' && <Card><CardHeader><CardTitle className="flex items-center gap-2 text-sm"><Plus size={16} /> Schedule interview</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
      <label><span className={labelClass}>Candidate</span><select className={inputClass} value={form.candidateId} onChange={(e) => setForm({ ...form, candidateId: e.target.value })}><option value="">Select candidate...</option>{candidates.map((c: any) => <option key={c._id} value={c._id}>{c.firstName} {c.lastName} - {c.jobRole}</option>)}</select></label>
    ...
    {view !== 'new' && view !== 'statistics' && <Card><CardContent className="grid gap-2 p-3 md:grid-cols-3"><label className="relative"><Filter className="absolute left-3 top-2.5 h-4 w-4 text-zinc-400" /><input className={`${inputClass} pl-9`} placeholder="Search round, mode, location or feedback..." value={query} onChange={(e) => setQuery(e.target.value)} /></label><select className={inputClass} value={status} onChange={(e) => setStatus(e.target.value)}><option>All</option><option>Scheduled</option><option>Completed</option><option>Cancelled</option><option>No_Show</option></select><select className={inputClass} value={round} onChange={(e) => setRound(e.target.value)}><option>All</option>{ROUND_TYPES.map((type) => <option key={type}>{type}</option>)}</select></CardContent></Card>}

    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-sm"><CalendarClock size={16} /> {view === 'statistics' ? 'Round breakdown' : `Interview register · ${pageMeta.total || visible.length} record${(pageMeta.total || visible.length) === 1 ? '' : 's'}`}</CardTitle></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-xs"><thead className="bg-zinc-50 text-zinc-500 dark:bg-zinc-900"><tr><th className="px-4 py-3 font-medium">Candidate</th><th className="px-4 py-3 font-medium">Role</th><th className="px-4 py-3 font-medium">Round</th><th className="px-4 py-3 font-medium">Interviewer</th><th className="px-4 py-3 font-medium">Schedule</th><th className="px-4 py-3 font-medium">Mode / Location</th><th className="px-4 py-3 font-medium">Status</th><th className="px-4 py-3 font-medium">Rating / Feedback</th><th className="px-4 py-3 font-medium text-right">Action</th></tr></thead><tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
      {isLoading && <tr><td colSpan={9} className="px-4 py-6 text-zinc-500">Loading interviews...</td></tr>}
      {visible.map((item: any) => {
        const candidate = item.candidateId || {};
        const interviewer = item.interviewerId || {};
        const editing = feedbackId === item._id;
        return <tr key={item._id} className="align-top"><td className="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">{candidate.firstName} {candidate.lastName}</td><td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{candidate.jobRole || '-'}</td><td className="px-4 py-3">{item.roundType}</td><td className="px-4 py-3">{interviewer.firstName ? `${interviewer.firstName} ${interviewer.lastName || ''}` : 'Unassigned'}</td><td className="px-4 py-3 whitespace-nowrap">{new Date(item.scheduledDate).toLocaleString()}</td><td className="px-4 py-3"><span className="block font-medium">{item.mode || '-'}</span><span className="block text-zinc-500">{item.location || item.meetingLink || '-'}</span></td><td className="px-4 py-3"><span className="rounded-full bg-zinc-100 px-2 py-1 text-[10px] font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">{item.status}</span></td><td className="px-4 py-3 max-w-[280px]">{editing ? <div className="grid gap-2"><select className={inputClass} value={feedback.status} onChange={(e) => setFeedback({ ...feedback, status: e.target.value })}><option>Completed</option><option>Cancelled</option><option>No_Show</option></select><select className={inputClass} value={feedback.rating} onChange={(e) => setFeedback({ ...feedback, rating: Number(e.target.value) })}>{[1, 2, 3, 4, 5].map((rating) => <option key={rating} value={rating}>{rating}/5</option>)}</select><textarea className={inputClass} rows={2} placeholder="Feedback notes" value={feedback.feedback} onChange={(e) => setFeedback({ ...feedback, feedback: e.target.value })} /></div> : <>{item.rating ? `${item.rating}/5` : '-'}{item.feedback && <p className="mt-1 whitespace-normal text-zinc-500">{item.feedback}</p>}</>}</td><td className="px-4 py-3 text-right">{item.status === 'Scheduled' && (editing ? <div className="flex justify-end gap-2"><Button size="sm" disabled={saveFeedback.isPending} onClick={() => saveFeedback.mutate(item._id)}><CheckCircle2 size={13} className="mr-1" />Save</Button><Button size="sm" variant="outline" onClick={() => setFeedbackId(null)}>Cancel</Button></div> : <Button size="sm" variant="outline" onClick={() => { setFeedbackId(item._id); setFeedback({ status: 'Completed', rating: item.rating || 3, feedback: item.feedback || '' }); }}>Feedback</Button>)}</td></tr>;
      })}
    ...
    </tbody></table></div>{view !== 'new' && <div className="flex items-center justify-between border-t border-zinc-100 px-4 py-3 text-xs dark:border-zinc-800"><span className="text-zinc-500">Showing {visible.length} of {pageMeta.total || visible.length}</span><div className="flex gap-2"><Button size="sm" variant="outline" disabled={page <= 1 || isLoading} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</Button><Button size="sm" variant="outline" disabled={page >= (pageMeta.totalPages || 1) || isLoading} onClick={() => setPage((value) => value + 1)}>Next</Button></div></div>}</CardContent></Card>
  </div>;
}
