...
export default function CandidateWorkflow({ candidateId }: { candidateId: string }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-1 pb-8">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col">
        ...
        {/* --- TABS NAVIGATION INSIDE HEADER CARD --- */}
        <div className="flex overflow-x-auto overflow-y-hidden border-t border-slate-100 bg-slate-50/50 hide-scrollbar px-3 pt-1">
          {['Overview', ...PHASES].map(tab => (
            <button
              key={tab}
      ...
      <div className="min-h-[400px]">
        ...
        {PHASES.map(phase => (
          activeTab === phase && (
            <div key={phase} className="animate-in fade-in duration-300">
              <Card className="border-slate-200 shadow-sm">
                      ...
                      return (
                        <div key={step.id} className="flex flex-col justify-between rounded-xl border border-slate-200 bg-slate-50/50 p-4 shadow-sm hover:shadow-md transition-shadow">
                          <div className="flex flex-col gap-3">
                            ...
                            {pipelineStep && pipelineStep.checklist && pipelineStep.checklist.length > 0 && (
                              <div className="mt-1">
                                <StepChecklist items={pipelineStep.checklist} />
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex shrink-0 flex-wrap gap-2 mt-4 pt-3 border-t border-slate-100 justify-end">
                            ...
                            {!disabled ? (
                              ...
                              <span className="rounded-md bg-slate-100 px-2 py-1 text-[10px] text-slate-500 border border-slate-200">Link employee</span>
                            )}
                          </div>
                        </div>
                      );
