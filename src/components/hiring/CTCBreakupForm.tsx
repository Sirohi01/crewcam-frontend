...
export default function CTCBreakupForm({ candidateId }: { candidateId: string }) {
  ...
  useEffect(() => {
    ...
    if (savedCTCRecords === undefined || selectionApprovals === undefined || candidate === undefined) return;

    const saved = editId ? savedCTCRecords?.find(r => r._id === editId) : savedCTCRecords?.[0];
    if (saved) {
      setForm(current => ({
        ...current,
    ...
    mutationFn: async () => {
      const payload = {
        ...
        breakup: {
          ...
          conveyance: 0,
          medicalAllowance: 0,
          specialAllowance: otherAllowance * 12,
          pfEmployer: employerPF * 12,
          pfEmployee: employeePF * 12,
          gratuity: 0,
          bonus: 0,
          otherAllowances: 0
        },
      ...
      const existing = editId ? savedCTCRecords?.find(r => r._id === editId) : savedCTCRecords?.[0];
      if (existing?._id) {
        ...
        <div className="p-3">
          <form onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="space-y-2">
            ...
            <div className="flex flex-col sm:flex-row justify-end items-center gap-2 pt-4">
              <button
                type="button"
                onClick={() => window.open(`/dashboard/hiring/${candidateId}/print/ctcBreakup`, '_blank')}
