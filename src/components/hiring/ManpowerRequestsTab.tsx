...
function Section({ number, title, children }: { number: string; title: string; children: React.ReactNode }) {
  return <section className="border-b border-slate-200 px-5 py-4 last:border-0 hover:bg-slate-50/30 transition-colors"><h2 className="mb-4 text-[11px] font-bold uppercase tracking-wider text-[#0d3c68]">{number}. {title}</h2>{children}</section>;
}
...
function LibraryControls({
  libraryItems, onPick, onSaveToLibrary, canSave, label: pickLabel,
}: {
  libraryItems: any[];
...
function AiGenerateBar({
  customPrompt, onCustomPromptChange, onGenerate, generating, error,
}: {
  customPrompt: string;
  ...
  return (
    <div className="mb-4 rounded-[4px] border border-indigo-100 bg-indigo-50/50 p-3">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] font-bold uppercase tracking-wider text-[#0d3c68]">AI Draft — Job Description &amp; KRA</span>
      </div>
      <p className="mt-1 text-[11px] text-slate-500">
        Generates the job description and KRA/KPIs together from the position selected above. Optionally add instructions to steer the draft, then click Generate — or leave it blank for a standard draft.
      </p>
      <textarea
        className="mt-2 h-14 w-full resize-y rounded-[2px] border border-slate-300 bg-white px-3 py-2 text-xs outline-none transition focus:border-[#0d3c68] focus:ring-1 focus:ring-[#0d3c68]"
        placeholder="Optional: e.g. 'emphasize client-facing communication' or 'this is a senior/lead-level role'"
        value={customPrompt}
      ...
      <button
        ...
        onClick={onGenerate}
        disabled={generating}
        className="mt-2 inline-flex h-7 items-center gap-1.5 rounded-[2px] border border-indigo-200 bg-white px-3 text-[11px] font-bold uppercase tracking-wider text-[#0d3c68] hover:bg-indigo-100 disabled:opacity-50"
      >
...
export default function ManpowerRequestsTab({ formOnly = false, requestId }: { formOnly?: boolean; requestId?: string }) {
  ...
  const generateJdKra = useMutation({
    mutationFn: async () => (await api.post('/ai/hiring/generate-jd-kra', aiRoleContext())).data,
    onSuccess: (data) => set({
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      workLocation: existing.workLocation || branchLocation(existing.locationBranchId),
