  ...
  const fetchTraining = async () => {
    try {
      ...
      if (tr) {
        ...
        router.push('/company/support/lms');
      }
    } catch (error) {
      console.error(error);
    } finally {
    ...
    <div className="flex flex-col h-[calc(100vh-80px)] -mt-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between gap-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center gap-3 min-w-0">
          <Link href="/company/support/lms" className="grid place-items-center h-9 w-9 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 shrink-0">
            <ArrowLeft size={18} />
          </Link>
