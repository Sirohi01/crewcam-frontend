  ...
  if (isError || !emp) {
    return (
      <div className="flex flex-col gap-3">
        <Button variant="ghost" className="w-fit text-xs h-7 px-2" onClick={() => router.push('/dashboard/employees')}>
    ...
    <div className="flex flex-col gap-2 animate-in fade-in duration-300 pb-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 relative">
          <Link href={`/dashboard/employees?edit=${emp._id}`}>
...
function AttendanceSummaryCard({ emp, cardClassName }: { emp: any; cardClassName?: string }) {
  ...
  return (
    ...
    >
      ...
      <Link href={`/dashboard/attendance/individual/${emp._id}`} className="block text-center text-[11px] text-indigo-600 hover:underline mt-2 shrink-0">
        View All Attendance
      </Link>
    </SectionCard>
  );
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <SectionCard
        ...
        action={<Link href={`/dashboard/attendance/individual/${emp._id}`} className="text-[11px] text-indigo-600 hover:underline">View All</Link>}
      >
