...
import {Download, Upload, Plus, Search, Filter, RotateCcw, ChevronDown, ChevronLeft, ChevronRight, Users, FileText, Star, Briefcase, XCircle, UserCheck, Eye, MessageSquare, MoreVertical, LayoutGrid, Loader2} from 'lucide-react';
...
export default function CandidateRegisterUI() {
  ...
  const filteredCandidates = React.useMemo(() => {
    ...
    return result;
  }, [candidates, status, department, experience, jobOpening, location, source, query, sortField]);
                ...
                <tr key={c.id} className="hover:bg-indigo-50/30 transition-colors group">
                  ...
                  <td className="px-2 py-2">
                    <div className="flex items-center justify-center gap-1.5">
                      <Link href={`/dashboard/hiring/candidates/${c.id}`} className="h-6 w-6 flex items-center justify-center rounded border border-indigo-100 text-indigo-700 hover:bg-indigo-50 bg-white shadow-sm transition-colors">
                        <Eye size={12} />
                      </Link>
...
//   Users, FileText, Star, Briefcase, XCircle, UserCheck, Eye, MessageSquare, MoreVertical, LayoutGrid, Loader2} from 'lucide-react';
// import api from '@/lib/axios';
...
//         (c.skills || []).some((s: string) => s.toLowerCase().includes(q)) || 
//         c.jobAppliedFor.toLowerCase().includes(q);
//       return matchStatus && matchDept && matchExp && matchQuery;
//     });
...
//     return result;
//   }, [candidates, status, department, experience, query, sortField]);
...
//               <Filter size={13} /> Filters
//             </button>
//             <button onClick={() => { setQuery(''); setStatus('All Status'); setDepartment('All Departments'); }} className="flex flex-1 md:flex-none items-center justify-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-[11px] font-semibold text-zinc-600 hover:bg-zinc-50 shadow-sm">
//               <RotateCcw size={13} /> Clear All
//             </button>
//           </div>
//         </div>
...
//                                   </td>
//                                 )}
//                                 {visibleCols.experience && <td className="px-2 py-2 font-medium text-zinc-700">{c.totalExperience ? `${c.totalExperience} Yrs` : '-'}</td>}
//                                 {visibleCols.skills && (
//                                   <td className="px-2 py-2">
//                                     <div className="flex flex-wrap gap-1 w-40 whitespace-normal">
//                                       {(c.skills || []).slice(0, 3).map((skill: string, idx: number) => (
...
//                                   </td>
//                                 )}
//                                 {visibleCols.stage && <td className="px-2 py-2 text-zinc-800 font-medium">{c.currentStage || c.pipelineStage || '-'}</td>}
//                                 {visibleCols.addedOn && (
//                                   <td className="px-2 py-2">
//                                     <div className="flex flex-col gap-0.5">
