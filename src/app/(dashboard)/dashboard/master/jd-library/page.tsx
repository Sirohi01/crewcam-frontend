...
const empty = {
  title: '', designation: '', jobDescriptionSummary: '', keyResponsibilitiesText: '',
  qualificationReq: '', experienceReq: '', technicalSkills: '', softSkills: '',
};
...
export default function JdLibraryPage() {
  ...
  const fetchData = async () => {
    ...
    setError('');
    try {
      const res = await api.get('/jd-library');
      setEntries(res.data || []);
    } catch (e: any) {
      setError(e.response?.data?.message || 'Failed to load JD library');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (item: any) => {
    ...
    setForm({
      ...
      jobDescriptionSummary: item.jobDescriptionSummary || '',
      keyResponsibilitiesText: (item.keyResponsibilities || []).join('\n'),
      qualificationReq: item.qualificationReq || '', experienceReq: item.experienceReq || '',
      technicalSkills: item.technicalSkills || '', softSkills: item.softSkills || '',
    });
    setModalItem(item);
    setModal(true);
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalItem?._id) await api.put(`/jd-library/${modalItem._id}`, payload);
      else await api.post('/jd-library', payload);
      setModal(false);
      await fetchData();
    } catch (e: any) {
  ...
  const executeDelete = async () => {
    ...
    setSaving(true);
    try {
      await api.delete(`/jd-library/${deleteConfirm.id}`);
  ...
  return (
    <div className="space-y-4 relative">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-md tracking-tight text-zinc-900 dark:text-zinc-50">JD Library</h1>
          <p className="text-xs text-zinc-500">Reusable job description templates — pick one directly in a Manpower Request, or generate a new one with AI and save it here.</p>
        </div>
        <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white" onClick={openCreate}>
          <Plus size={14} className="mr-1.5" /> Add JD
        </Button>
      </div>
      ...
      <Card className="border-zinc-200 shadow-sm dark:border-zinc-800">
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
            ...
            {!loading && entries.length === 0 && <div className="p-8 text-center text-sm text-zinc-500">No JD templates yet.</div>}
            {!loading && entries.map((item) => (
              <div key={item._id} className="px-5 py-3 flex items-start justify-between gap-3 hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-zinc-500 mt-0.5 line-clamp-2">{item.jobDescriptionSummary}</p>
                </div>
      ...
      {deleteConfirm && (
        <ConfirmModal title={`Delete ${deleteConfirm.title}?`} busy={saving} onCancel={() => setDeleteConfirm(null)} onConfirm={executeDelete}>
          This JD template will be removed from the library.
        </ConfirmModal>
      )}
