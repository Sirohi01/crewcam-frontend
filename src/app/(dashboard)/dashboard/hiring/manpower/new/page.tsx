...
const inputCls = 'mt-1 h-7 w-full rounded-none border border-zinc-200 bg-white px-2.5 text-[11.5px] text-zinc-800 outline-none transition focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400';
const selectCls = `${inputCls} appearance-none`;
const textAreaCls = 'mt-1 w-full resize-y rounded-none border border-zinc-200 bg-white px-2.5 py-1 text-[11.5px] text-zinc-800 outline-none transition focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400';
const labelCls = 'text-[11px] font-semibold text-zinc-600';
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white shadow-sm">
      <div className="flex items-center gap-2.5 border-b border-zinc-100 px-4 py-3">
        <span className="grid h-6 w-6 shrink-0 place-items-center rounded-md bg-indigo-600 text-[11px] font-bold text-white">{number}</span>
  ...
  return (
    <div className="bg-[#fafbfc] font-sans">
      <div className="mx-auto max-w-[1500px] p-1">
        {/* Left column (header + KPIs + form) / Right column (actions + sidebar cards) — both start from the top */}
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[3fr_1fr]">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              {KPIS.map((s) => (
                <div key={s.label} className="rounded-xl border border-zinc-200 bg-white p-3.5 shadow-sm">
                  <div className="flex items-center gap-2.5">
                    <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${s.accent}`}>
                      <s.icon size={17} />
                    </span>
          ...
          <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
            ...
            <SectionCard number={2} title="Job Details">
              <div className="grid grid-cols-1 gap-1.5 lg:grid-cols-[3fr_2fr]">
                ...
                <div className="rounded border border-zinc-200">
                  <div className="border-b border-zinc-100 px-2 py-1">
                    <span className="text-[11.5px] font-semibold text-zinc-700">Key Performance Indicators (KPIs) <b className="text-rose-500">*</b></span>
                  </div>
                  <div className="divide-y divide-zinc-100">
                    {kpiRows.map((k) => (
                      <div key={k.name} className="flex items-center gap-2 px-2 py-0.5">
                        <span className="min-w-0 flex-1 truncate text-[11px] text-zinc-700">{k.name}</span>
                        <input className="h-6 w-12 rounded border border-zinc-200 px-1.5 text-center text-[11px]" defaultValue={k.weight} />
                        <span className="text-[10.5px] text-zinc-400">%</span>
                      </div>
                    ))}
                  </div>
                  <button type="button" className="flex w-full items-center gap-1.5 px-2 py-1 text-[11px] font-semibold text-indigo-600 hover:bg-indigo-50/50">
            ...
            <SectionCard number={4} title="Compensation Details">
              ...
              <Field title="Other Benefits">
                <div className="mt-1 flex flex-wrap items-center gap-1 rounded border border-zinc-200 bg-white px-2.5 py-1.5">
                  {otherBenefits.map((b) => (
                    <span key={b} className="flex items-center gap-1 rounded-full bg-indigo-50 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-700">
            ...
            <SectionCard number={5} title="Attachments (Optional)">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 flex h-16 flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed border-zinc-200 text-center">
                ...
                <Field title="Notes (Optional)" hint="0 / 500">
                  <textarea className={`${textAreaCls} h-16`} placeholder="Add any additional information or notes..." />
                </Field>
              </div>
            </SectionCard>
          </form>
          ...
          <div className="space-y-3">
            <div className="mt-3 mb-1 flex gap-2">
              <button type="button" className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-[11.5px] font-semibold text-zinc-700 shadow-sm hover:bg-zinc-50">
                <Save size={14} /> Save as Draft
              </button>
              <button type="button" className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-indigo-600 px-2.5 py-1.5 text-[11.5px] font-semibold text-white shadow-sm hover:bg-indigo-700">
            ...
            >
              <p className="mb-1.5 text-[11px] leading-snug text-zinc-500">Let AI help you create a complete and accurate job requisition.</p>
              <div className="space-y-1">
                {aiActions.map((a) => (
                  <button key={a.title} type="button" className="flex w-full items-start gap-2 rounded-lg border border-zinc-100 px-2 py-1.5 text-left hover:bg-indigo-50/40">
              ...
              <button type="button" className="mt-1.5 flex w-full items-center justify-center gap-1.5 border-t border-zinc-100 pt-1.5 text-[11px] font-semibold text-indigo-600 hover:text-indigo-700">
            ...
            <div className="flex items-center gap-2.5 rounded-xl border border-zinc-200 bg-white p-3 shadow-sm">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-indigo-600 text-white">
                <Bot size={17} />
              </span>
              <p className="text-[11px] leading-snug text-zinc-600">Hi Swati! Need help in generating JD or KPIs?</p>
            </div>
          </div>
        </div>
      </div>
