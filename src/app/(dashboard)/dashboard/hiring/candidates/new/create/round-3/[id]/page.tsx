...
const DUMMY_QUESTIONS = [
  ...
  { category: 'Stakeholder Management', text: 'Tell me about a time you had to push back on a key stakeholder\'s request. How did you handle it?', insight: 'Evaluates negotiation skills and ability to manage expectations.' },
  { category: 'Resource Allocation', text: 'How do you decide where to allocate limited budget and headcount in your department?', insight: 'Tests financial acumen and strategic resource planning.' }
];
...
export default function InterviewUI() {
  ...
  const [activeQuestions, setActiveQuestions] = React.useState<Question[]>(DUMMY_QUESTIONS);
  ...
  const handleManualOverride = async (idx: number, verdict: string) => {
    try {
      const updatedQuestions = [...activeQuestions];
      const prevAnalysis = updatedQuestions[idx].answerAnalysis || { reasoning: '' };
      updatedQuestions[idx].answerAnalysis = { ...prevAnalysis, verdict };
      setActiveQuestions(updatedQuestions);

      if (interviewId) {
        const dbQuestions = updatedQuestions.map(q => ({
          question: q.text,
          transcript: q.transcript || answers[updatedQuestions.indexOf(q)] || '',
          answerAnalysis: q.answerAnalysis
        }));
        await api.put(`/hiring/interviews/${interviewId}/questions`, { questions: dbQuestions });
        toast.success('Manual rating saved');
      }
    } catch (err) {
      console.error(err);
      toast.error('Failed to save manual rating');
    }
  };
  const handleBulkSave = async () => {
    ...
    try {
      setIsSaving(true);
      const updatedActiveQuestions = activeQuestions.map((q: any, idx: number) => ({
        ...q,
        answerAnalysis: {
          verdict: answers[idx]?.length > 50 ? 'adequate' : (answers[idx]?.length > 0 ? 'weak' : 'no_answer'),
          reasoning: q.insight
        }
      }));
      setActiveQuestions(updatedActiveQuestions);
      
      const dbQuestions = updatedActiveQuestions.map((q: any, idx: number) => ({
        question: q.text,
  ...
  const initInterview = async () => {
    ...
    try {
      ...
      // 1. Fetch or create interview
      const { data: interviews } = await api.get(`/hiring/interviews/${candidate._id}`);
      let activeInterview = interviews.find((i: any) => i.roundType === 'Managerial');
      ...
      if (activeInterview.interviewQuestions && activeInterview.interviewQuestions.length > 0) {
        ...
        // Trigger AI Generation based on Candidate Experience and JD
        try {
          const genRes = await api.post(`/hiring/interviews/${activeInterview._id}/generate-questions`);
          ...
          if (generatedQuestions.length > 0) {
            const mappedQuestions = generatedQuestions.map((q: string) => ({
              category: 'Managerial',
              text: q,
              insight: 'Evaluates candidate response based on managerial context.'
            }));

            // Save them to the database
            const dbQuestions = mappedQuestions.map((q: any) => ({
              question: q.text,
            ...
            setActiveQuestions(mappedQuestions);
            setAnswers(Array(mappedQuestions.length).fill(''));
          } else {
            throw new Error("No questions generated");
          }
        } catch (genErr) {
          console.error("AI Generation failed, falling back to dummy questions", genErr);
      ...
      setActiveQuestions(DUMMY_QUESTIONS);
      setAnswers(Array(DUMMY_QUESTIONS.length).fill(''));
      toast.success('Interview Session Started (Standard Mode)');
    }
  };
  ...
  const handleEndExam = async (isAutoSubmit = false) => {
    try {
      ...
      if (interviewId) {
        await api.put(`/hiring/interviews/${interviewId}/feedback`, { status: 'Completed', rating: 0, feedback: 'Completed via AI Round 3' });
      }
      setIsCompleted(true);
      setActiveTab('AI Questions');
      setIsAiConnected(false);
      ...
      <div className="w-full max-w-[1600px] px-2 py-1 mx-auto space-y-2 font-sans text-zinc-900 min-h-screen">
        ...
        <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4  pb-2">
          <div>
            ...
            <p className="text-[11px] text-zinc-500 mt-0.5">Managerial Interview – AI Powered</p>
          </div>
        ...
        <div className="flex items-center gap-6 border-b border-zinc-200 px-2 overflow-x-auto whitespace-nowrap scrollbar-hide">
          {['Interview', 'AI Questions', 'Notes', 'Attachments', 'Score'].map((tab) => (
            <button
              ...
              className={`pb-2 text-[12px] font-bold border-b-2 transition-colors ${activeTab === tab
              ...
              <div className="lg:col-span-6 flex flex-col gap-4 h-full">
                <div className="p-5 rounded-xl border border-zinc-100 bg-white shadow-sm flex flex-col h-full">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={toggleFlag} className={`flex items-center gap-1.5 text-[10px] font-semibold border px-2 py-1 rounded transition-colors shadow-sm ${flaggedQuestions.includes(currentQuestionIndex)
                ...
                <div className="bg-[#f8f7ff] rounded-xl p-4">
                  ...
                  <div className="flex flex-col gap-3">
                    {[
                      ...
                      "Answers are analyzed for managerial accuracy, depth & clarity.",
                ...
                <div className="p-5 rounded-xl border border-zinc-100 bg-white shadow-sm flex flex-col">
                  ...
                  <div className="relative pl-3 flex flex-col gap-5">
                    ...
                    <div className="relative z-10 flex items-start gap-3">
                      <div className="h-5 w-5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-500 flex items-center justify-center shrink-0 mt-0.5"><Check size={10} strokeWidth={3} /></div>
                      <div className="flex flex-col flex-1">
                        <span className="text-[10px] font-bold text-zinc-800">Round 2</span>
                        <span className="text-[9px] text-zinc-500">Technical Interview</span>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">Completed</span>
                        <span className="text-[9px] text-zinc-500 font-medium">40 Min</span>
                      </div>
                    </div>
                    ...
                    <div className="ml-7 bg-zinc-50 p-3 rounded-lg border border-zinc-200">
                      ...
                      <div className="bg-white border border-zinc-200 rounded p-3 flex flex-col gap-2 mt-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`text-[9px] font-bold px-2 py-0.5 rounded ${q.answerAnalysis?.verdict === 'adequate' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                        ...
                        <div className="mt-2 pt-2 border-t border-zinc-100 flex items-center justify-between">
                          ...
                          <select
                            className="text-[9px] border border-zinc-200 rounded px-2 py-1 bg-zinc-50 focus:outline-none"
                            value={q.answerAnalysis?.verdict || ''}
                            onChange={(e) => handleManualOverride(idx, e.target.value)}
                          >
        ...
        {activeTab === 'Score' && (
          <div className="mt-4 p-8 rounded-xl border border-zinc-200 bg-white shadow-sm flex flex-col items-center justify-center text-center min-h-[400px]">
            ...
            <h3 className="text-[18px] font-bold text-zinc-900 mb-2">Round 3 Test Score</h3>
            ...
            <button
              onClick={() => {
                ...
                try {
                  ...
                  storedMap[candidateId] = Math.max(storedMap[candidateId] || 0, 3);
                  localStorage.setItem('ai_completed_rounds_map', JSON.stringify(storedMap));
                } catch (e) {}
                router.push(`/dashboard/hiring/candidates/new/create/round-4/${candidateId}`);
              }}
